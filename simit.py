    ...
    def _goto_with_retry(self, page, url, retries=3, timeout=60000):
        for attempt in range(retries):
            try:
                ...
                self.logger.warning(
                    f"Timeout on attempt {attempt+1} of {retries}")
        return False

    def scrape(self, nuip: str) -> Optional[RegistraduriaData]:
        try:
            with self._get_browser() as page:
                if not self._goto_with_retry(page, self.URL):
                    self.logger.error("All attempts to navigate failed.")
                    return None
                ...
                try:
                    page.wait_for_selector(
                        self.BANNER_CLOSE_SELECTOR, timeout=5000, state='visible')
                    page.click(self.BANNER_CLOSE_SELECTOR)
                    self.logger.info("Banner cerrado.")
                except PlaywrightTimeoutError:
                    self.logger.info(
                        "No se encontró el banner o ya está cerrado.")
                except Exception as e:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error al ingresar NUIP o clicar el botón: {e}")
                    return None
                ...
                try:
                    ...
                    try:
                        page.wait_for_selector(
                            f'xpath={self.RESULTADOS_XPATH}', timeout=10000, state='visible')
                        estado_element = page.query_selector(
                            f'xpath={self.RESULTADOS_XPATH}')
                        estado_text = estado_element.inner_text().strip() if estado_element else None
                        self.logger.debug(
                            f"Texto extraído del XPath principal: {estado_text}")
                        ...
                        self.logger.warning(
                            "XPath principal no encontrado, intentando alternativo")
                        estado_text = None

                    # Si no se encontró en el primer XPath, intentar con el alternativo
                    try:
                        page.wait_for_selector(
                            f'xpath={self.RESULTADOS_XPATH_ALTERNATIVE}', timeout=10000, state='visible')
                        alt_element = page.query_selector(
                            f'xpath={self.RESULTADOS_XPATH_ALTERNATIVE}')
                        estado_text = alt_element.inner_text().strip() if alt_element else None
                        self.logger.debug(
                            f"Texto extraído del XPath alternativo: {estado_text}")
                    ...
                    if not estado_text:
                        self.logger.warning(
                            "No se encontró información en ningún XPath")
                        return None
